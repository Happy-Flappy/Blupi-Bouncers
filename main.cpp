...
public:
    ...
    void update(RenderWindow &window)
    {
        ...
        if (status == glueball)
        {
            ...
            sprite.setTextureRect(IntRect(210, 0, 16, 16));
        ...
        if (status == tank)
        {
            sprite.setTextureRect(IntRect(567, 668, 57, 59));
            ...
            if (sprite.getTextureRect() != IntRect(0, 609, 56, 58))
            {
        ...
        if (status == jeep)
        {
            sprite.setTextureRect(IntRect(384, 441, 54, 54));
        ...
        if (status == fly)
        {
            sprite.setTextureRect(IntRect(408, 846, 45, 60));
...
//                   	}
        ...
        {
            if (sprite.getPosition().y + (sprite.getTextureRect().height / 2) < workarea.height)
            {
                velocity.y += gravity;
            }
            else
            {
            	velocity.y = 0;
                sprite.setPosition(sprite.getPosition().x,workarea.height-(sprite.getTextureRect().height / 2));
...
int main()
{
    ...
    while (mainwindow.isOpen())
    {
		...
		Vector2f MPosition = mainwindow.mapPixelToCoords(Mouse::getPosition(mainwindow));
        ...
        mainwindow.clear(Color(255, 0, 255));
